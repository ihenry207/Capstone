...
def Format_Voltage(actual):
    voltage = (actual / 1023.0) * 3.3
...
while True:
    led.off()
    actual = chip.read(0)
    formatted_Voltage = Format_Voltage(actual)
    oled.text(f"ADC Value: {actual}",0,0)
    ...
    oled.show()
    actual = chip.read(0)
    formatted_Voltageb = Format_Voltage(actual)
    oled.text(f"ADC Value: {actual}",0,0)#top left
    oled.text(f"Voltage: {formatted_Voltage}", 0, 30)#middle left
    oled.show()
    utime.sleep(0.5)
    oled.fill(0) #clear whatever is there
